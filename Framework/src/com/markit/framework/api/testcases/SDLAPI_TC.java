...
public class SDLAPI_TC extends SDLAPI {
	...
	public void tc_getSDLTimestampAPI(String token,String timestamp,String next,int totalCountRecords) throws ClientProtocolException, IOException, InterruptedException, JSONException,
	KeyManagementException, NoSuchAlgorithmException, KeyStoreException {
		...
		ReportUtil.reportElementException("RelationshipList not found: "+e.getMessage(), LogAs.FAILED);	
		}
		...
		if(countPerPage==100){		  
			ReportUtil.reportWebElement("Total No. of records per page in Timestamp: "+countPerPage, true);	
			}

		totalCountRecords=totalCountRecords+countPerPage;
		String ts=null;
		String nxt=null;
		try{
			...
			ReportUtil.reportWebElement("Total No. of records on last page page in Timestamp: "+countPerPage, true);	
			ReportUtil.reportWebElement("Total No. of records in Timestamp: "+totalCountRecords, true);
		}		
		
		Thread.sleep(10000);
		if(ts!=null && nxt!=null){
		tc_getSDLTimestampAPI(token,ts,nxt,totalCountRecords);
		}
	}
	...
	public void tc_getSDLTimestampAPI_EmptyRelList(String token,String timestamp,String next) throws KeyManagementException, NoSuchAlgorithmException, KeyStoreException, ClientProtocolException, IOException, JSONException{
		...
		try{
		rowssArray=root.getJSONArray("relationshipList");
		if(rowssArray.length()==0)
		ReportUtil.reportWebElement("RelationshipList found Empty: ", true);
		else{
		ReportUtil.reportElementException("RelationshipList found with list of records: " +rowssArray.length(), LogAs.FAILED);	
		}
